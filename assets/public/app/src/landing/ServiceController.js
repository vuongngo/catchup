...
.controller('ServiceController', ['$scope', '$geolocation', 'Services', function($scope, $geolocation, Services, $moment) {
	...
	$geolocation.getCurrentPosition({
	    ...
	    Services.get_stops($scope.myLocation).success(function(res) {
	    	...
	    	for (i=0; i < $scope.locations.length; i ++) {
	    		...
	    		$scope.locations[i].needTime = Math.round( (par / (60 * $scope.travel)) * 10) / 10;
	    	};
	    }).error(function(err) {
	    	console.log(err);
	    })	
  ...
  $scope.getDepartures = function(loc) {
  	...
  	Services.get_departures($scope.departureInfo).success(function(res) {
    	...
    	// for (i=0; i < $scope.departures.length; i ++) {
    	// 	console.log($moment($scope.departures[i].time_realtime_utc).fromNow());
    	// 	$scope.departures[i].time_realtime_utc = $moment($scope.departures[i].time_realtime_utc).fromNow();
    	// };
  	}).error(function(err) {
  		console.log(err);
  	})
